...
class Pool extends appolo_event_dispatcher_1.EventDispatcher {
    constructor(options) {
        ...
        this._threads = [];
        this.options = appolo_utils_1.Objects.defaults(this.options, { maxThreadJobs: Number.MAX_SAFE_INTEGER });
    ...
    _onError(e, thread) {
        appolo_utils_1.Arrays.removeBy(this._threads, item => item === thread);
    ...
    async _checkForJob() {
        ...
        let threads = appolo_utils_1.Arrays.sortBy(this._threads, (thread) => thread.runningJobs);
