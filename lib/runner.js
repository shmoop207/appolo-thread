...
class Runner {
    ...
    async _start(path) {
        try {
            ...
            this._worker = new WorkerCtr(worker_threads_1.workerData);
    ...
    async _run(data) {
        try {
            ...
            this._port.postMessage({ action: action_1.Action.RunSuccess, result });
        }
        catch (e) {
            this._port.postMessage({ action: action_1.Action.RunFail, error: e.stack });
        }
    }
}
